...
- \`@Language({ id, extensions?, aliases?, configuration?, grammar? })\` + \`@Hover\`/\`@Completion\`/\`@CodeLens\`/\`@Diagnostics\`/\`@InlineCompletion\`/\`@CodeAction\`/\`@Definition\`/\`@References\`/\`@Rename\`/\`@Formatting\`/\`@Symbols\`/\`@InlayHints\` — **DSL própria: declare \`extensions: [".minha"]\`** e o sigil emite o \`contributes.languages\` (sem isso o VSCode nunca associa o arquivo ao id e nada dispara); \`grammar: "./syntaxes/x.tmLanguage.json"\` = highlight; \`@Formatting\` pode retornar o documento formatado como **string**; CodeLens/Diagnostics que refletem estado FORA do doc: chame \`registry.languages.get("MinhaClasse").refresh()\` após mudar o estado
